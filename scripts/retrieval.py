...
from dkmodel2vec.retrieval import (
    add_embeddings,
    add_embeddings_wrapped,
    retrieve,
    retrieve_bm25s,
    create_corpus,
    add_recall,
    get_mapping_from_query_to_corpus,
    add_corpus_idx,
    retrieve_hybrid,
...
def retrieve_and_calculate_recall(queries, flat_corpus, top_k=30, index_name="m2v"):
    ...
    queries_with_retrieval = queries.map(
        lambda examples: retrieve(
            examples, flat_corpus, top_k=top_k, index_name=index_name
        ),
        batched=True,
        batch_size=100,
    ...
    queries_with_retrieval = queries_with_retrieval.map(
        add_recall, load_from_cache_file=False
    )
...
def run_e5_model(ds_test, e5_model):
    ...
    flat_corpus = flat_corpus.map(
        lambda examples: {
            "embeddings": e5_model.encode(
                examples["document"], normalize_embeddings=True
            )
        },
        batched=True,
        batch_size=500,
    ...
    retrieval_time, recall_metrics = retrieve_and_calculate_recall(
        ...
        flat_corpus,
        top_k=30,
        index_name="e5",  # ← Add index_name="e5"
    )
...
def run_bm25s(ds_test, flat_corpus, query2corpus, tokenizer_model):
    ...
    # create special tokenizer required by BM25s from actual tokenizer
    t0 = time()
...
def run_hybrid_model(ds_test, embedding_model, model_name, rrf_k=60):
    ...
    mlflow.log_metric(
        "corpus_encoding_seconds", corpus_encoding_time + bm25_encoding_time
    )

    # Encode queries for embeddings
    t0 = time()
    queries = add_embeddings_wrapped(
        ds=queries, model=embedding_model, in_column="query", out_column="embeddings"
    )
    query_encoding_time = time() - t0
    ...
    queries_with_retrieval = queries.map(
        lambda examples: retrieve_hybrid(
            examples, flat_corpus, bm25_retriever, top_k=30, rrf_k=rrf_k
        ),
        batched=True,
        batch_size=100,
...
def run_two_stage_model(
    ...
    flat_corpus.add_faiss_index(
        column="embeddings", index_name="cheap", string_factory="HNSW32", metric_type=1
    )
    ...
    mlflow.log_metric(
        "stage1_corpus_encoding_seconds", cheap_encoding_time + bm25_encoding_time
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run retrieval evaluation with variable models"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to static embedding model to evaluate retrieval for.",
    )
    ...
    with mlflow.start_run(run_name="llm2model2vec"):
        ...
        run_embedding_model(ds_test, model, model_name=args.model, index_name="m2v")
