...
def encode_and_save_incrementally(
    model, ds_filtered, output_path, batch_size=32, chunk_size=5000
):
    """
    ...
    for chunk_idx, chunk_start in enumerate(
        tqdm(range(0, total_examples, chunk_size), desc="Encoding chunks")
    ):
        ...
        chunk_embeddings = model.encode(
            chunk, batch_size=batch_size, convert_to_numpy=True, show_progress_bar=False
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Featurize texts using an LLM2Vec encoder and save as Dataset."
    )
    parser.add_argument(
        "--max-means",
        type=int,
    ...
    with mlflow.start_run():
        ...
        output_path = (
            ...
            / "features"
            / f"features_{max_means}_max_length_{max_length_arg}"
        )
        ...
        ds = ds.map(
            lambda batch: check_fits_length(
                batch, tokenizer, model_max_length, doc_max_length
            ),
            batched=True,
            batch_size=10000,
            num_proc=1,
        ...
        logger.info(
            f"Kept {filtered_size:,} / {len(ds):,} examples ({filter_ratio * 100:.1f}%)"
        )
        ...
        mlflow.log_metric(
            "examples_per_second",
            filtered_size / encoding_time if encoding_time > 0 else 0,
        )
        ...
        logger.info(
            f"Encoding speed: {filtered_size / encoding_time:.2f} examples/second"
        )
