...
def train_model(
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        if strip_exotic:
            ...
            token_remove_pattern = r"|".join(
                [token_remove_pattern, contains_exotic_pattern]
            )
        if strip_uncommon:
            contains_uncommon_pattern = CONTAINS_UNCOMMON_PATTERN
            token_remove_pattern = r"|".join(
                [token_remove_pattern, contains_uncommon_pattern]
            )

        # Create vocabulary
        if not ignore_external_tokens:
            logger.info(f"Creating vocabulary with size {vocab_size}...")
            vocabulary = create_vocabulary(texts, vocab_size=vocab_size, stem=stem)
        else:
            vocabulary = None
        # Train model
...
def main():
    ...
    parser.add_argument(
        "--no-save", action="store_true", help="Don't save the trained model"
    )
    ...
    parser.add_argument(
        "--stem", action="store_true", help="Use Danish stemmer. (default: False)"
    )
