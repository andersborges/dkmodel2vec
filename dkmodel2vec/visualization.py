...
class TokenEmbeddingVisualizer:
    ...
    def _extract_static_embeddings(
        ...
        for string_idx, (text, embeddings_array) in enumerate(
            zip(strings, embeddings_list)
        ):
            tokens = self.model.tokenizer.encode(text).tokens
            token_data = []

            for token_idx, (token, embedding) in enumerate(
                zip(tokens, embeddings_array)
            ):
                token_data.append(
                    {
                        "token": token,
    ...
    def _extract_llm2vec_embeddings(
        self, strings: List[str], exclude_special_tokens: bool = True
    ) -> Tuple[List[List[Dict]], List[str], Optional[List[np.ndarray]]]:
        """
    ...
    def _compute_chained_positions(
        self, embeddings_2d: np.ndarray, start_pos: Tuple[float, float] = (0.0, 0.0)
    ) -> np.ndarray:
        """
    ...
    def visualize(
        ...
        if self.model_type == "static":
            ...
            tokens_data, all_tokens, attention_matrices = (
                self._extract_llm2vec_embeddings(strings, exclude_special_tokens)
            )

        # Collect all token embeddings (unscaled) for PCA
        all_token_embeddings = []
        all_scaled_contributions = []
        all_mean_embeddings = []
        string_token_counts = []

        for token_list in tokens_data:
            token_embeddings = np.array([item["embedding"] for item in token_list])
            n_tokens = len(token_embeddings)
            string_token_counts.append(n_tokens)

            # Scaled contributions
            scaled_contributions = token_embeddings / n_tokens
            for scaled_emb in scaled_contributions:
                all_scaled_contributions.append(scaled_emb)

            # Mean embedding
            mean_embedding = token_embeddings.mean(axis=0)
            all_mean_embeddings.append(mean_embedding)

        # Fit PCA on scaled token embeddings only
        self.pca.fit(np.array(all_scaled_contributions))
        explained_var = self.pca.explained_variance_ratio_

        # Transform scaled contributions and mean embeddings
        scaled_contributions_2d = self.pca.transform(np.array(all_scaled_contributions))
        # Transform mean embeddings and add back the PCA mean to "uncenter" them
        pca_mean_2d = self.pca.transform(self.pca.mean_.reshape(1, -1))[0]
        mean_embeddings_2d = self.pca.transform(np.array(all_mean_embeddings))
        mean_embeddings_2d = mean_embeddings_2d  # - pca_mean_2d  # Shift mean embeddings to align with chains starting at origin
        # Generate colors
        ...
        for string_idx, (token_data, color) in enumerate(zip(tokens_data, colors)):
            n_tokens = string_token_counts[string_idx]

            # Get the scaled contributions for this string in 2D
            string_contributions_2d = scaled_contributions_2d[
                contribution_idx : contribution_idx + n_tokens
            ]
            contribution_idx += n_tokens

            # Transform the PCA mean to 2D to use as starting point
            pca_mean_2d = self.pca.transform(self.pca.mean_.reshape(1, -1))[0]

            # Compute chained positions starting from the PCA mean
            start_x = pca_mean_2d[0]
            start_y = pca_mean_2d[1]
            positions = self._compute_chained_positions(
                string_contributions_2d, (start_x, start_y)
            )
        ...
        if show_attention and attention_matrices is not None:
            for string_idx, (attention_matrix, midpoints) in enumerate(
                zip(attention_matrices, all_midpoints)
            ):
        ...
        # Add mean embedding markers
        for string_idx, (color, mean_embedding_2d) in enumerate(
            zip(colors, mean_embeddings_2d)
        ):
            fig.add_trace(
                go.Scatter(
                    x=[mean_embedding_2d[0]],
                    y=[mean_embedding_2d[1]],
                    mode="markers",
                    marker=dict(
                        size=5,
                        color=color,
        ...
        if title is None:
            model_name = (
                "LLM2Vec" if self.model_type == "llm2vec" else "Static Embedding"
            )
            attention_note = " with Bidirectional Attention" if show_attention else ""
            title = (
                f"{model_name} Token Embedding Visualization{attention_note}<br>"
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                title="PC2", showgrid=True, zeroline=True, scaleanchor="x", scaleratio=1
            ),
            hovermode="closest",
            showlegend=True,
