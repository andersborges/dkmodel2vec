...
def create_corpus(
    examples: dict,
    columns: list[str] = [
        constants.DATASET_POSITIVE_COLUMN,
        constants.DATASET_NEGATIVE_COLUMN,
    ],
):
        ...
        for c in columns:
            if examples[c][n] is not None:
                if examples[c][n].strip() != "":
                    document_n = {
                        "query_idx": examples["idx"][n],
                        "document": examples[c][n],
                        "column": c,
                    }
...
def add_embeddings_wrapped(
    ds: Dataset,
    model: SentenceTransformer | LLM2Vec,
    in_column: str,
    out_column: str,
    batch_size: int = 1000,
    device: str = "cuda:0",
    ...
    ds = ds.map(
        lambda examples: {
            out_column: model.encode(
                examples[in_column], normalize_embeddings=True, device=device
            )
        },
        batched=True,
...
def retrieve(
    ...
    scores, retrieved_examples = index.get_nearest_examples_batch(
        index_name=index_name, queries=queries, k=top_k
    )
...
def retrieve_bm25s(
    query_examples: dict[str, list], retriever: BM25, top_k: int = 30
) -> dict:
...
def retrieve_hybrid(
    query_examples, flat_corpus, bm25_retriever, top_k=30, index_name="m2v", rrf_k=60
):
    """
    ...
    for i in range(len(emb_results["retrieved_document_idx"])):
        ...
        # RRF fusion
        fused_ranking = reciprocal_rank_fusion([emb_ranking, bm25_ranking], k=rrf_k)
...
def create_fresh_corpus_and_queries(ds: Dataset):
    ...
    queries = ds.map(
        lambda example: {
            "query": example["query"],
            "idx": example["idx"],
            "query_instruct": example["query_instruct"],
        },
        remove_columns=ds.column_names,
        load_from_cache_file=False,
    )
